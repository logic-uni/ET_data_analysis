...
def collect_isi_data(units):
    all_histograms = []
    valid_unit_ids = []
    spike_counts = []  # 新增发放次数记录
    ...
    # 解包排序后的数据
    _, valid_unit_ids, all_histograms = zip(*sorted_data)
...
def plot_3d_isi(units):
    ...
    for i, (unit_id, counts) in enumerate(zip(unit_ids, histograms)):
        ...
        ax.bar3d(xs,
                ...
                counts,
                alpha=ALPHA,
                edgecolor=None,  # 取消边框颜色
                linewidth=0)     # 取消边框线宽


    # 轴标签优化
    ax.set_xlabel('Neuron ID', labelpad=15, fontsize=12)
    ...
    # 视角调整：正对ISI轴
    ax.view_init(elev=15,  # 俯仰角
                azim=-30)   # 方位角设为0度（正对Y轴）
